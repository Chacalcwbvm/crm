...
if ($total_rows > 0) {
    ...
    switch ($relatorio_tipo) {
        case 'mensal':
            $report .= "<tr><th>Mês</th><th>Total</th></tr>";
            break;
        case 'anual':
            $report .= "<tr><th>Ano</th><th>Total</th></tr>";
            break;
        case 'semanal':
            $report .= "<tr><th>Semana</th><th>Total</th></tr>";
            break;
        case 'diario':
        default:
            $report .= "<tr><th>Data de Atendimento</th><th>Total</th></tr>";
            break;
    }
    while ($row = $result->fetch_assoc()) {
        ...
        switch ($relatorio_tipo) {
            case 'mensal':
                $report .= "<td>" . date('M/Y', strtotime($row['data_atendimento'])) . "</td>";
                break;
            case 'anual':
                $report .= "<td>" . date('Y', strtotime($row['data_atendimento'])) . "</td>";
                break;
            case 'semanal':
                $report .= "<td>" . date('W/Y', strtotime($row['data_atendimento'])) . "</td>";
                break;
            case 'diario':
            default:
                $report .= "<td>" . date('d/m/Y', strtotime($row['data_atendimento'])) . "</td>";
                break;
        }
    ...
    echo json_encode(['report' => "Nenhum atendimento encontrado."]);
}
